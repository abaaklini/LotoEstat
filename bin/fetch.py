...
def update_db (game='quina'):
    ...
    if filedb in list_of_files_inside_zip_file:
        ...
        fd = open('./temp/' + filedb, 'w')
        fd.write(zip_file.read(filedb))
        fd.close()

        if not os.path.exists('./data/' + filedb):
            f = file('./data/' + filedb, "w")
            f.close()

        if not filecmp.cmp('./temp/' + filedb,
                './data/' + filedb):
            logger.info('Arquivo %(arq)s sendo atualizado' % {'arq': filedb}) 
            shutil.copyfile('./temp/' + filedb,
                    './data/' + filedb)
        else:
